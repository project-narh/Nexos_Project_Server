...
//////////////////////////////////////////////////////
	...
	#include_with_pragmas"../Inc/Common.hlsl"
	...
	#ifdef MK_LENS_FLARE
		inline float CreateHalo(float2 uv, float2 halfUV, float2 offset, float2 size, half intensity, half fade)
		{
			return pow(1.0 - length(halfUV - frac(uv + offset + size)) / length(halfUV), fade) * intensity;
		}
	#endif
	...
	HEADER
	{
		...
		float2 foveatedHalfUV = UV_HALF;
		#if defined(SUPPORTS_FOVEATED_RENDERING_NON_UNIFORM_RASTER)
			UNITY_BRANCH
			if(_FOVEATED_RENDERING_NON_UNIFORM_RASTER)
			{
				BLOOM_UV = RemapFoveatedRenderingLinearToNonUniform(BLOOM_UV);
				foveatedHalfUV = RemapFoveatedRenderingLinearToNonUniform(foveatedHalfUV);
			}
		#endif
		...
		half4 source = Presample(PASS_TEXTURE_2D(_SourceTex, sampler_linear_clamp_SourceTex), BLOOM_UV, SOURCE_TEXEL_SIZE); //Bloom is always presampled
			...
			{ 
				float2 offset = frac(LENS_FLARE_UV + (foveatedHalfUV - LENS_FLARE_UV) * LENS_FLARE_GHOST_DISPERSAL * i);

				half weight = pow(1.0 - length(foveatedHalfUV - offset) / length(foveatedHalfUV), LENS_FLARE_GHOST_FADE);
			...
			float weight;
			#ifdef UNITY_SINGLE_PASS_STEREO
				haloSize = normalize(foveatedHalfUV - LENS_FLARE_UV) * LENS_FLARE_HALO_SIZE * 0.5;
				float4 haloSizeSP = float4(normalize(foveatedHalfUV - LENS_FLARE_UV - STEREO_OFFSET),normalize(foveatedHalfUV - LENS_FLARE_UV + STEREO_OFFSET)) * LENS_FLARE_HALO_SIZE;
				weight = CreateHalo(LENS_FLARE_UV, foveatedHalfUV, STEREO_OFFSET, haloSizeSP.xy * 0.5, LENS_FLARE_HALO_INTENSITY * 0.5, LENS_FLARE_HALO_FADE * 2.0);
				weight += CreateHalo(LENS_FLARE_UV, foveatedHalfUV, -STEREO_OFFSET, haloSizeSP.zw * 0.5, LENS_FLARE_HALO_INTENSITY * 0.5, LENS_FLARE_HALO_FADE * 2.0);
			#else
				haloSize = normalize(foveatedHalfUV - LENS_FLARE_UV) * LENS_FLARE_HALO_SIZE;
				half2 haloSizeSP = normalize(foveatedHalfUV - LENS_FLARE_UV) * LENS_FLARE_HALO_SIZE;
				weight = CreateHalo(LENS_FLARE_UV, foveatedHalfUV, 0, haloSizeSP, LENS_FLARE_HALO_INTENSITY, LENS_FLARE_HALO_FADE);
			#endif
