...
public class S_PlayerList : IPacket
{
    ...
    public class Player
    {
        ...
        {
            if (s.Length < count + sizeof(bool) + sizeof(int) * 2 + sizeof(float) * 7)
            {
                return;
            }
    ...
    public void Read(ArraySegment<byte> segment)
    {
        ...
        if (s.Length < sizeof(ushort) * 3)
        {
            return;
        }

        count += sizeof(ushort); // 전체 패킷 크기
        count += sizeof(ushort); // 패킷 ID
        this.sequenceNumber = BitConverter.ToUInt16(s.Slice(count, sizeof(ushort)));
        count += sizeof(ushort); // 시퀀스 번호

        // 플레이어 개수 읽기
        if (s.Length < count + sizeof(ushort))
        {
            return;
        }

        ushort playerLen = BitConverter.ToUInt16(s.Slice(count, sizeof(ushort)));
        count += sizeof(ushort);
