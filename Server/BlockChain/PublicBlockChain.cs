...
namespace Server.BlockChain
{
    ...
    new ConcurrentDictionary<int, TransactionMetadata>();// 서명 대기 트랜잭션
        ...
        bool isSign = false; // 테스트를 위함 : Sign을 넣은 퍼블릭과 가스비와 시간차이를 확인하기 위함
        ...
        public async Task<bool> Request(TransactionMetadata data, FunctionMessage func)
        {
            await ClearStuckNonceIfNeeded();
            ...
            if(func == null)
            {
                Console.WriteLine("[BlockChain_public] 트랜잭션 함수가 null입니다.");
                Console.WriteLine("[BlockChain_public] 작업을 중지합니다.");
                return false;
            }

            (TransactionMetadata meta, FunctionMessage _func) = await SetTransactionData(data, func, userAddress);
            if (meta == null)
            {
            ...
            {
                {
                    ...
                    try
                    {
                        ...
                        Console.WriteLine($"[BlockChain_public] 서버 계정 ETH 잔액: {await _web3Http.Eth.GetBalance.SendRequestAsync(_accountManager._serverAccount_public.Address)}");
                        var gameServerAddress = await _web3Http.Eth.GetContractQueryHandler<GameServerAddressFunction>()
                        .QueryAsync<string>(_contractAddress);
                        Console.WriteLine($"[BlockChain_public] 컨트랙트에 등록된 게임 서버 주소: {gameServerAddress}");
                        if (gameServerAddress.Equals(_accountManager._serverAccount_public.Address, StringComparison.OrdinalIgnoreCase))
                        {
                            Console.WriteLine("[BlockChain_public] 게임 서버 주소 일치 확인됨");
                        }
                        else
                        {
                            Console.WriteLine("[BlockChain_public] 경고: 게임 서버 주소가 일치하지 않습니다! onlyGameServer 제한에 걸릴 수 있습니다.");
                            return false;
                        }
                        ...
                        if (_func is MintitemFunction_sign mintFuncTyped)
                        {
                            (isSuccess) = await SendTransactionAsync<MintitemFunction_sign>(mintFuncTyped, meta, finalNonce, maxFee, priorityFee);
                        }
                        else if (_func is MintitemFunction _mintFuncTyped)
                        {
                            (isSuccess) = await SendTransactionAsync<MintitemFunction>(_mintFuncTyped, meta, finalNonce, maxFee, priorityFee);
                        }
                        else if (_func is ItemBurnFunction burnFuncTyped)
                        {
                            (isSuccess) = await SendTransactionAsync<ItemBurnFunction>(burnFuncTyped, meta, finalNonce, maxFee, priorityFee);
                        }
                        else if (_func is TradeItemsByServerFunction tradeFuncTyped)
                        {
                            (isSuccess) = await SendTransactionAsync<TradeItemsByServerFunction>(tradeFuncTyped, meta, finalNonce, maxFee, priorityFee);
                        }
                        else if (_func is TradeItemsByServerFunction_Sign _tradeFuncTyped)
                        {
                            (isSuccess) = await SendTransactionAsync<TradeItemsByServerFunction_Sign>(_tradeFuncTyped, meta, finalNonce, maxFee, priorityFee);
                        }
                        if (isSuccess)
                            Console.WriteLine($"[BlockChain_public] 예상 가스비: {UnitConversion.Convert.FromWei(meta.GasLimit * maxFee, UnitConversion.EthUnit.Ether)} ETH");
                        else
                        {
                            Console.WriteLine($"[BlockChain_public] 트랜잭션 전송 실패");
                            return false;
                        }
                    }
                    catch (Exception ex)
                ...
                //Star
                //0._SendToClient(data.uid, _txHash);
        ...
        private async Task<bool> SendTransactionAsync<T>(T funcTyped, TransactionMetadata meta, HexBigInteger nonce, BigInteger maxFee, HexBigInteger priorityFee) where T : FunctionMessage, new()
        {
            try
            {
                Console.WriteLine($"[BlockChain_public] {typeof(T).Name} 트랜잭션");
                funcTyped.FromAddress = _accountManager._serverAccount_public.Address;
                funcTyped.Nonce = nonce;
                funcTyped.MaxFeePerGas = new HexBigInteger(maxFee);
                funcTyped.MaxPriorityFeePerGas = priorityFee;
                funcTyped.Gas = meta.GasLimit;

                Console.WriteLine($"[BlockChain_public] {typeof(T).Name} 트랜잭션 데이터: {BitConverter.ToString(funcTyped.GetCallData()).Replace("-", "")}");

                var _tx = await _contractHandler.SendRequestAsync(funcTyped);
                Console.WriteLine($"[BlockChain_public] {typeof(T).Name} 트랜잭션 전송됨: {_tx}");
            ...
            {
                Console.WriteLine($"[BlockChain_public] {typeof(T).Name} 트랜잭션 실패: {ex}");
                return (false);
            }
        }
        ...
        public async Task<(TransactionMetadata, FunctionMessage)> SetTransactionData(TransactionMetadata data, FunctionMessage func, string address)
        {
            byte[] functionDataBytes;
            try
            {
                Console.WriteLine($"[BlockChain_public] NFT 트랜잭션 데이터 재구성");

                if(func is MintitemFunction_sign mintSignFunc)
                {
                    Console.WriteLine($"[BlockChain_public] Mint 확인");
                    if (!isSign)
                    {
                        var converted = new MintitemFunction(mintSignFunc);
                        converted.to = address;
                        converted.dataHash = GetItemDataHash(data);
                        functionDataBytes = converted.GetCallData();
                        func = converted;
                    }
                    else
                    {
                        ...
                        mintSignFunc.dataHash = dataHash;
                        mintSignFunc.signature = signature;
                        mintSignFunc.to = address;
                        functionDataBytes = mintSignFunc.GetCallData();
                    }
                    data.From = _accountManager._serverAccount_public.Address;
                }
                else if (func is ItemBurnFunction burnFunc)
                {
                    Console.WriteLine($"[BlockChain_public] burn 확인");
                    burnFunc.from = address;
                    data.From = _accountManager._serverAccount_public.Address; ;
                    functionDataBytes = burnFunc.GetCallData();
                }
                else if (func is TradeItemsByServerFunction_Sign transferFunc)
                {
                    Console.WriteLine($"[BlockChain_public] Trade 확인");
                    if (!isSign)
                    {
                        var converted = new TradeItemsByServerFunction(transferFunc);
                        converted.User1 = (await DBManager.Instance.GetAccountByPrivateAddress(transferFunc.TradeInfo.User1)).address;
                        converted.User2 = (await DBManager.Instance.GetAccountByPrivateAddress(transferFunc.TradeInfo.User2)).address;
                        functionDataBytes = converted.GetCallData();
                        func = converted;
                    }
                    else
                    {
                        transferFunc.TradeInfo.User1 = (await DBManager.Instance.GetAccountByPrivateAddress(transferFunc.TradeInfo.User1)).address;
                        transferFunc.TradeInfo.User2 = (await DBManager.Instance.GetAccountByPrivateAddress(transferFunc.TradeInfo.User2)).address;
                        functionDataBytes = transferFunc.GetCallData();
                    }
                    data.From = _accountManager._serverAccount_public.Address;
                }
                else
                {
                    Console.WriteLine($"[BlockChain_public] 잘못된 형태 확인");
